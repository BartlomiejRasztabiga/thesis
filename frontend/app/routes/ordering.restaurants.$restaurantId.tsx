...
export default function RestaurantPage() {
  ...
  let activeOrderRestaurantSelected =
    data.activeOrder && data.restaurant.id === data.activeOrder.restaurantId;
  ...
  const getContent = () => {
    ...
    if (!activeOrderRestaurantSelected)
      return (
        <>
          <p className="text-lg text-center mb-4">
            You have an active order from another restaurant
          </p>
          <Link
            to={`/ordering/restaurants/${data.activeOrder.restaurantId}`}
  ...
  return (
    <div className="flex h-full bg-white">
      ...
      <div className="h-full w-80 border-r bg-gray-50">{getContent()}</div>
    </div>
  );
}
